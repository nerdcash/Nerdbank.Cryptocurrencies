...
public interface IExchangeRateProvider
{
	...
	/// <param name="acceptableBases">The securities that may be used to purchase the <paramref name="tradeInterest"/>.</param>
	/// <param name="cancellationToken">A cancellation token.</param>
	/// <returns>The trading pair that was found; or <see langword="null" /> if no match was found.</returns>
	async ValueTask<TradingPair?> FindFirstSupportedTradingPairAsync(Security tradeInterest, IReadOnlyList<Security> acceptableBases, CancellationToken cancellationToken)
	{
