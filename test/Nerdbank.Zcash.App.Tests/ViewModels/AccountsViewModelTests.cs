...
public class AccountsViewModelTests : ViewModelTestBase
{
	...
	{
		HDWallet hd = Assert.Single(this.MainViewModel.Wallet.HDWallets);
		Zip32HDWallet zip32 = hd.GetZip32HDWalletByNetwork(network);
		...
		Assert.Same(zip32, newAccount2.ZcashAccount.HDDerivation?.Wallet);
	...
	public async Task NewAccount_WithMultipleHDWallets()
	{
		...
		HDWallet firstHD = Assert.Single(this.MainViewModel.Wallet.HDWallets);
